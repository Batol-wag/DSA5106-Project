...
class RepVGGBlockWithBranches(nn.Module):
    ...
    def get_equivalent_kernel_bias(
        ...
        """
        Compute the fused 3x3 kernel and bias for deploy conversion.
        """
        if self.deploy:
    ...
    def switch_to_deploy(self) -> None:
        """
        Fuse the training-time branches into a single 3x3 conv.
        """
        if self.deploy:
            return
...
class RepVGGWithBranches(nn.Module):
    ...
    def __init__(
        ...
        self.in_channels = min(64, int(32 * self.width_multiplier[0]))
        ...
        self.stage1 = self._make_stage(
            out_channels=int(32 * self.width_multiplier[0]),
            num_blocks=num_blocks[0],
            stride=1,
        )

        self.stage2 = self._make_stage(
            out_channels=int(64 * self.width_multiplier[1]),
            num_blocks=num_blocks[1],
            stride=2,
        )

        self.stage3 = self._make_stage(
            out_channels=int(128 * self.width_multiplier[2]),
            num_blocks=num_blocks[2],
            stride=2,
        )

        self.stage4 = self._make_stage(
            out_channels=int(256 * self.width_multiplier[3]),
            num_blocks=num_blocks[3],
            stride=2,
        )

        self.gap = nn.AdaptiveAvgPool2d(output_size=1)
        self.linear = nn.Linear(int(256 * self.width_multiplier[3]), num_classes)
    ...
    def switch_to_deploy(self) -> None:
        """
        Convert all blocks to deploy mode (no branching).
        """
