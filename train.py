...
Main training script for CIFAR-10 image classification experiments.

This file manages the full training workflow, including:

1. Parsing command-line arguments for model, training, saving,
   and augmentation settings.

2. Preparing CIFAR-10 dataloaders with training and validation
   preprocessing pipelines.

3. Supporting training-time regularization and augmentation methods:
   - Random crop
   - Horizontal flip
   - Cutout
   - Mixup
...
4. Building one of the supported models:
   - RepVGG
   - ResNet-18
   - ResNet-34

5. Running the training loop with:
   - Mixed precision training (AMP)
   - SGD optimizer
   - Cosine annealing learning rate schedule
   - Validation after each epoch

6. Saving outputs for later analysis and reuse:
   - Best checkpoint
   - Final checkpoint
   - Training history in JSON format

7. Reporting metrics such as training loss, validation loss,
   Top-1 accuracy, Top-5 accuracy, learning rate, and epoch time.

Used for reproduction experiments, baseline comparison,
and final project training runs.
"""
...
def str2bool(value: str) -> bool:
    """
    Convert common string inputs into a boolean value.
    """
...
class Cutout:
    """
    Apply Cutout augmentation by masking one or more square regions
    ...
    def __call__(self, img: torch.Tensor) -> torch.Tensor:
        """
        Randomly mask square patches in the image tensor.
        """
        height = img.size(1)
        width = img.size(2)
...
def apply_repvgg_preset(args: argparse.Namespace) -> argparse.Namespace:
    """
    Apply preset width settings for RepVGG variants.
    """
...
def resolve_defaults(args: argparse.Namespace) -> argparse.Namespace:
    """
    Adjust augmentation-related defaults based on the selected mode.
    """
...
def get_input_size() -> tuple[int, int, int, int]:
    """
    Return the default input size used for CIFAR-10 model summary and FLOPs.
    """
    return (1, 3, 32, 32)
...
def _select_subset(
    ...
    """
    Select a reproducible subset of a dataset based on fraction
    and optional maximum sample count.
    """
...
def build_cifar10_train_transform(args: argparse.Namespace) -> transforms.Compose:
    """
    Build the training transform pipeline for CIFAR-10.
    """
    transform_list: list = [
...
def build_cifar10_val_transform() -> transforms.Compose:
    """
    Build the validation transform pipeline for CIFAR-10.
    """
...
def get_dataloaders(args: argparse.Namespace) -> tuple[DataLoader, DataLoader]:
    ...
    """
    train_transform = build_cifar10_train_transform(args)
...
def mixup_criterion(
    ...
    """
    Compute the mixup loss as the weighted sum of two label losses.
    """
...
def build_run_name(args: argparse.Namespace) -> str:
    """
    Construct a descriptive run name for saved outputs.
    """
    if args.model == "repvgg":
...
def get_save_paths(args: argparse.Namespace) -> dict[str, Path]:
    """
    Create and return file paths for checkpoints and history files.
    """
...
def save_checkpoint(
    ...
    """
    Save model, optimizer, scheduler, and training metadata to a checkpoint file.
    """
...
def train_one_epoch(
    ...
    """
    Train the model for one epoch and return average loss and Top-1 accuracy.
    """
    model.train()
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with open(save_paths["history"], "w", encoding="utf-8") as file:
